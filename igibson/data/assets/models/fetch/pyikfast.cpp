...
static struct PyModuleDef pyikfast_fetchmodule = {
...
PyMODINIT_FUNC PyInit_pyikfast_fetch(void)
{
  PyObject *module = PyModule_Create(&pyikfast_fetchmodule);
  return module;
}
...
PyObject *inverse(PyObject *self, PyObject *args)
{
  ...
  // Parse arguments
  if (!PyArg_ParseTuple(args, "OOd", &argTranslation, &argRotation, &argFree))
  {
    return NULL;
  }
  for (int i = 0; i < 3; i++)
  ...
  free_joint[0] = argFree;


  // Compute inverse kinematics
  IkSolutionList<IkReal> solutions;
  ComputeIk(translation, rotation, free_joint, solutions);
